...
//	specular_tex_color.w = (specular_tex_color.w * -1) + 1;
	for	(int i = 0; i < LIGHT_POINT_MAX; i++){
		if(lights[i].enabled){
			diffuse += lights[i].getDiffuse(frag_Normal_n, frag_Pos, texture(_albedo, texCoord) * _color_diffuse);
...
//RESULT
//	out_Color = vec4(texCoord.r, texCoord.g, 0, 0);
out_Color =
